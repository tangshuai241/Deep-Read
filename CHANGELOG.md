...
| 更新内容 | 说明 |
|------|------|
| 笔记按用户隔离 | 新增 `note.isolate_by_user`，开启后笔记写入 `notes/users/<user_id>/《书名》/` |
| Trial 默认隔离 | Trial/IM-first 默认开启，朋友试用不会把笔记混进同一个目录 |
| Personal 兼容 | Personal 默认关闭，个人 Obsidian 主库路径不被强行加 `users/` 层 |
| Agent 写笔记透传用户 | `write_note` 工具调用自动带 `--user <飞书 open_id>`，状态、契约、会话、笔记统一按用户分层 |
| Web 用户筛选 | 首页、会话页、笔记页增加用户维度，可按用户查看当前阅读、会话和笔记 |
