...
def init_contract(args):
    ...
    # 尝试从 4D 拆解预读分析播种知识地图
    _seed_from_pre_read(contract, getattr(args, "book", ""))

    path = save_contract(contract, args.user)
    return {"ok": True, "path": str(path), "contract": contract}


def _seed_from_pre_read(contract, book_name):
    """从 4D 拆解输出播种知识地图，不覆盖已手动设置的知识点。"""
    if not book_name:
        return
    try:
        from preload_analysis import find_analysis_dir, load_4d_json, generate_seeds
        ...
        if not books_dir:
            # No books_dir configured — skip preload gracefully
            return

        analysis_dir = find_analysis_dir(books_dir, book_name)
        if not analysis_dir:
            return  # 无预读分析，不干预

        data = load_4d_json(analysis_dir)
        seeds = generate_seeds(data)

        # 只补充尚为空的知识地图分组
        for group in POINT_GROUPS:
            existing = contract.get("knowledge_map", {}).get(group, [])
            if not existing and seeds.get(group):
                contract["knowledge_map"][group] = normalize_points(seeds[group])

    except ImportError:
        pass  # preload_analysis 不可用时不阻断
    except Exception:
        pass  # 预读播种失败不阻断契约初始化
